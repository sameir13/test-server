...
const Page = ({ params }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await axios.get(`/api/tips/${params.slug}`);
      setTricksData(res?.data?.message);
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if(file || tricksData?.imgUrl){
      try {
        setIsFetching(true);
        const imageURL = await UploadImageToCloudinary();
        const res = await axios.put(`/api/tips/${params.slug}`, {
          ...tricksData,
          imgUrl: imageURL,
          author: user?._id,
        });
                  ...
                  {(file || tricksData?.imgUrl ) ? (
                    <div className=" relative border border-slate-300 p-3 rounded-md my-3 ">
                      <i
                        ...
                        onClick={() => tricksData?.imgUrl ? setTricksData({imgUrl:null}) : setFile(null)}
                      ></i>
                      <Image
                        className=" w-full  h-auto object-contain aspect-[3/2]  "
                        src={ tricksData?.imgUrl ? tricksData?.imgUrl : URL.createObjectURL(file)}
                        alt=""
                        width={500}
                        height={400}
                      ></Image>
